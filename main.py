...
class WarpSimulator:
    ...
    def show_gif_and_result(self, resultado):
        ...
        if resultado == "imagens/firefly_pull.png":
            gif = AnimatedGif(new_window, "imagens/5_estrelas.gif", loop=False, on_complete=lambda: self.show_result(new_window, resultado))
            gif.pack()
        else:
            gif = AnimatedGif(new_window, "imagens/3_estrelas.gif", loop=False, on_complete=lambda: self.show_result(new_window, resultado))
            gif.pack()

    def show_result(self, window, image_path):
    ...
    def proximo(self, result_window, label):
        result_image = ImageTk.PhotoImage(Image.open("imagens/welt_pull.png"))
        self.image_references.append(result_image)
